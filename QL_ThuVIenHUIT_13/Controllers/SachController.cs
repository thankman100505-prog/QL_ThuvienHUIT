...
namespace QL_ThuVIenHUIT_13.Controllers
{
    ...
    {
        // Đảm bảo tên Context đúng với trong Web.config
        private CNPM_DATABASE_THUVIENEntities1 db = new CNPM_DATABASE_THUVIENEntities1();

        [ChildActionOnly]
        public ActionResult _SidebarPartial()
        {
            // Cache nhẹ hoặc lấy danh sách thể loại đơn giản
            return PartialView(db.THELOAI.ToList());
        }

        public ActionResult Index(string tuKhoa = "")
        {
            // Eager loading: Load luôn Bìa, Tác giả để tránh query lặp (N+1 problem)
            var query = db.QLSACH
                          .Include(s => s.TACGIA)
                          .Include(s => s.BIASACH); // Quan trọng: Include bảng mới
        ...
        public ActionResult ChiTiet(string maSach)
        {
            ...
            ViewBag.SanPhamLienQuan = db.QLSACH
                                        .Include(s => s.BIASACH)
        ...
        {
            var listSach = db.QLSACH.Include(s => s.TACGIA)
                                    .Include(s => s.THELOAI)
                                    .Include(s => s.NHAXUATBAN)
                                    .Include(s => s.BIASACH)
                                    .OrderByDescending(s => s.MASACH) // Sách mới nhập lên đầu
                                    .ToList();
        ...
        public ActionResult CapNhat(string maSach)
        {
            ...
            var sach = db.QLSACH.Include(s => s.TACGIA)
                                .Include(s => s.THELOAI)
                                .Include(s => s.NHAXUATBAN)
                                .Include(s => s.BIASACH)
        ...
        public ActionResult CapNhat(QLSACH sach, string tenTacGia, string tenTheLoai, string tenNXB, HttpPostedFileBase fileAnh)
        {
            ...
            if (ModelState.IsValid)
            {
                using (var transaction = db.Database.BeginTransaction())
                {
                    try
                    {
                        var sachInDb = db.QLSACH.Include(s => s.BIASACH).FirstOrDefault(s => s.MASACH == sach.MASACH);
                        ...
                        if (sachInDb != null)
                        {
                            ...
                            if (fileAnh != null && fileAnh.ContentLength > 0)
                            {
                                ...
                                var biaSach = db.BIASACH.FirstOrDefault(b => b.MASACH == sachInDb.MASACH);
                                if (biaSach != null) biaSach.URL_ANH = fileName;
                                else db.BIASACH.Add(new Models.BIASACH { MASACH = sachInDb.MASACH, URL_ANH = fileName });
                            }
            ...
            var currentBook = db.QLSACH.Include(s => s.TACGIA).Include(s => s.BIASACH).FirstOrDefault(s => s.MASACH == sach.MASACH);
        ...
        public ActionResult XoaSach(string maSach)
        {
            try
            {
                var sach = db.QLSACH.Find(maSach);
                if (sach == null) return HttpNotFound();

                var bia = db.BIASACH.FirstOrDefault(b => b.MASACH == maSach);
                if (bia != null)
                {
                    db.BIASACH.Remove(bia);
                }
                db.QLSACH.Remove(sach);
                db.SaveChanges();
